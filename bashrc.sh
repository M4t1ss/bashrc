...
# colors
export PS1='${debian_chroot:+($debian_chroot)}\[\033[01;36m\]\u@\[\e[m\]\[\e[01;37m\]\h\[\e[m\]:\[\033[01;33m\]\w\[\033[00m\]\$\[\033[01;32m\] '
...
myLocalHistory() {
  if test -w .
  then
    history 1 | awk '($2 !~ "^[mr]?cd[0-9a-z]?$") {$1="_T="strftime("%Y%m%d_%H:%M:%S_") PROCINFO["ppid"] "\t"; $2=gensub("^_T=[-_0-9:]*[ \t]* *", "", 1, $2); $2=gensub("^_P=[^ \t]* *", "", 1, $2); print;}' >>.history-$LOGNAME
  else
    history 1 | awk '($2 !~ "^[mr]?cd[0-9a-z]?$") {$1="_T="strftime("%Y%m%d_%H:%M:%S_") PROCINFO["ppid"] "_PWD="  ENVIRON["PWD"] "\t"; $2=gensub("^_T=[-_0-9:]*[ \t]* *", "", 1, $2); $2=gensub("^_P=[^ \t]* *", "", 1, $2); print;}' >> ~/.history-all-$LOGNAME
  fi
}
